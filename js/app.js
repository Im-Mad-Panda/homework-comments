...
function renderCardShowcase(cardsDataArr, cardShowcaseEl) {
  ...
  smsCounterUnseenEl.textContent = cardsDataArr.filter(
    (cardDataObj) => !cardDataObj.seen
  ).length;
...
function createCardHTML(cardDataObj) {
  return `<div class="comments ${cardDataObj.seen ? "seen" : ""}">
    <img class="comments-img" src="${cardDataObj.avatar}" alt="${
    cardDataObj.name
  }" width="1" height="1" loading="lazy" decoding="async">
    <div class="comments-content">
    <h2 class="comments-author">${cardDataObj.name}</h2>
    <a class="comments-link" href="tel:${cardDataObj.phone}">Тел: ${
    cardDataObj.phone
  }</a>
    <p class="comments-text">${cardDataObj.text}
    </p>
  <p class="comments-date">Добавлено: ${new Date(
    cardDataObj.date
  ).toLocaleString()}</p>
  </div>
  </div> `;
